...
/**
 * @description 深圳爱屋吉屋整租房源抓取规则
 * @classname 深圳爱屋吉屋整租(k-ok)
 */
Class IwjwRent extends \city\PublicClass{
    ...
    Public function house_page(){
         ...
         * 爱屋及屋用搜索条件的方式获取页面列表
         */
        $dis=array(56001=>'南山区',56002=>'福田区',56003=>'罗湖区',56004=>'盐田区',60684=>'宝安区',60715=>'龙岗区',64345=>'龙华区',70399=>'光明新区',70400=>'坪山新区',70401=>'大鹏新区'
        );
        $rooms = array(1=>'一室',2=>'二室',3=>'三室',4=>'四室',5=>'四室以上');
        $url = array();
        foreach ($dis as $k => $v){
            foreach ($rooms as $m => $n){
                $url_tmp = 'http://www.iwjw.com/chuzu/shenzhen/g1id'.$k.'rn'.$m;
                $html = $this->getUrlContent($url_tmp);
                preg_match("/相关在租房源([\x{0000}-\x{ffff}]*?)套/u", $html, $total);
                $total = trimall(strip_tags($total[1]));
                $maxpage = ceil($total/30);
                for($page = 1; $page <= $maxpage; $page++){
                    $url[] = 'http://www.iwjw.com/chuzu/shenzhen/g1id'.$k.'rn'.$m.'p'.$page;
                }
            }
        }
        return $url;
