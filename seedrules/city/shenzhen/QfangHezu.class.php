...
Class QfangHezu extends \city\PublicClass{
    Public function house_page(){
        ...
        $maxPage=6;
        $urlarr = array();
        $url_pre="http://shenzhen.qfang.com/rent/h2-f-f";
    ...
    */
    public function house_list($url){
    	...
    	foreach($lis[0] as $k=>$li){
            ...
            $house_info[] = 'http://shenzhen.qfang.com/rent/'.$href[1].'|'.$tagstr.'|'.$href[1];
    	}
        return $house_info;
    }
...
//        $source_url = "http://shenzhen.qfang.com/rent/12220309|深圳市福田区景龙小学#距离2号线(蛇口线)景田站约251米#拎包入住#|12220309";
        $house_info = array();
        $split = explode('|',$source_url);
        $house_info['source_url'] = $split[0];
        $details = $this->getUrlContent($split[0]);
        //下架检测
//        $house_info['off_type'] = $this->is_off($split[0]);
    	preg_match('/<h2\sclass=\"text\_of\">([\x{0000}-\x{ffff}]*?)<\/h2>/u',$details,$title);
    	//标题
    	$house_info['house_title'] = $title[1];
    	preg_match('/guide-alink-inner([\x{0000}-\x{ffff}]*?)<\/div>/u',$details,$area);
    	...
    	//联系人姓名
    	preg_match('/broker-basic-name\">([\x{0000}-\x{ffff}]*?)<\/a>/u',$details,$name);
