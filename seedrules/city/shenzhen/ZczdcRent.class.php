...
/**
 * @description 深圳中诚致整租房抓取规则
 * @classname 深圳中诚致 (May-OK)
 */

Class ZczdcRent extends \city\PublicClass{

    public function house_page(){
        //从端口抓取
        //翻页规律：$Parameters ["start"]=0,1,2…..  $Parameters ["length"]=10(每一页抓取的数量可自定义)
        $minPage = empty ( $minPage ) ? 1 : $minPage;
        $maxPage = empty ( $maxPage ) ? 2 : $maxPage;
        $urlarr = [];
        for($page = $minPage; $page < $maxPage; $page ++) {
            //$url = "http://www.zczdc.com/hourselist.aspx?hotword=&jzlx=%E7%A7%9F%E6%88%BF&qy=&sjfw=&FX=&mzfw=&title=";
            //$page在里面没有翻页作用！待修改！！！
            $urlarr[] = "http://www.zczdc.com/hourselist.aspx?hotword=&jzlx=%E7%A7%9F%E6%88%BF&qy=&sjfw=&FX=&mzfw=&title=";
        }
        return $urlarr;
    }
...
//        $url = 'http://www.zczdc.com/hourselist.aspx?hotword=&jzlx=%E7%A7%9F%E6%88%BF&qy=&sjfw=&FX=&mzfw=&title=';
    ...
    public function house_detail($source_url){
        ...
        //房源id
// 	    preg_match("/编号：([\x{0000}-\x{ffff}]*?)发布/u", $title[1], $houseId);
// 	    $house_info['house_id'] = trimall($houseId[1]);
        ...
        //装修
        preg_match("/(毛坯|普装|精装修|豪装)/u", $info, $fitment);
        ...
        $desc = str_replace('&nbsp;','',trimall(strip_tags($desc[1])));
