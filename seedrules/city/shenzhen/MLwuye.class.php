...
/**
 * @description 深圳美联物业二手房抓取规则
 * @classname 深圳 =======《美联物业》=======深圳
 */

Class MLwuye extends \city\PublicClass
{

    public function house_page()
    {
            $url = 'http://www.1200.com.cn/sale/queryListData?pageSize=10&curPage=1&listingType=SALE&propertyType=apartment';
        $json = json_decode($this->getUrlContent($url), 1);
        $maxPage = intval($json['count'] / 10) + 1;
        $urlarr = [];
        for ($page = 1; $page <= $maxPage; $page++) {
            $urlarr[] = "http://www.1200.com.cn/sale/queryListData?pageSize=10&curPage=" . $page . "&listingType=SALE&propertyType=apartment";
        }
        return $urlarr;
    }
    ...
    public function house_list($url)
    {
        ...
        foreach ($result as $res) {
            $house_info[] = "http://www.1200.com.cn/sale/getInfoById/" . $res['id'] . "?type=esf";
        }
        return $house_info;
    }
...
//        $house_info["off_type"] = $this->is_off($source_url,$html);
        ...
        //价格
        preg_match("/总价：([\x{0000}-\x{ffff}]*?)万元/u", $info, $price);
        ...
        //小区名称
        preg_match("/楼盘名称：([\x{0000}-\x{ffff}]*?)楼盘/u", $info, $borough);
        $borough = explode("(", $borough[1]);
        $borough = explode("（", $borough[0]);
    ...
    public function callNewData()
    {
        ...
        for ($page = 1; $page <= 100; $page++) {
            $data[] = "http://www.1200.com.cn/sale/queryListData?pageSize=10&curPage=" . $page . "&listingType=SALE&propertyType=apartment";
        }
        return $data;
    }
}