...
/** 
 * @description 北京丁丁整租抓取规则
 * @classname 北京丁丁
 * @author lp
 * @version 1
 * @since 2016-03-24(DONE)
 */
...
class DingdingRent extends \city\PublicClass{
    ...
    Public function house_page() {
		$PRE_URL = 'http://bj.zufangzi.com/area/0-10000000000/';
		$data = $this->queryList($PRE_URL, [
			'total' => ['.navbar-wrapper .pull-right .color-yellow','text'],
		]);
		$total = $data[0]['total'];
		$maxPage = ceil($total/20);
		$urlarr=array();
		for($page = 1; $page <= $maxPage; $page++){
			$urlarr[] = "http://bj.zufangzi.com/area/0-10000000000/".$page;
		}
		return $urlarr;
    }
