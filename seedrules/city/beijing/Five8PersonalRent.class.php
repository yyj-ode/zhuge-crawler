	...
	public function house_page(){
		...
		for($index=0;$index<count($dis);$index++){
			for($page=1;$page<=$page_list[$index];$page++){
 				$urlarr[]= 'http://m.58.com/'.$dis[$index].'/zufang/0/pn'.$page.'?58ihm=m_house_index_zufang&58cid=1&PGTID=0d200001-0000-13c5-8a32-39bbbf1c61de&ClickID=1';
			}
		}
    	return $urlarr;
	}
...
//        preg_match("/户型([\x{0000}-\x{ffff}]+?)<\/li>/u",$html,$layout);
		...
		//获取加密电话号码
		if (substr_count($house_info['owner_phone'],'*')>0) {
			preg_match("/\d{14}/",$source_url,$infoid);
			$getphoneurl = 'http://app.58.com/api/windex/scandetail/car/'.$infoid[0].'/?pid=799&pagetype=detail&topcate=house&list_name=zufang&local_name=bj&infoID='.$infoid[0].'&jumptype=native&title=%E8%AF%A6%E6%83%85&pid=799&full_path=1,8&wlmode=sc&wltype=bro&wlsour=firefox#app';
			$html = file_get_contents($getphoneurl);
			preg_match("/(data-tel=\").*?\">/",$html,$phone);
			preg_match_all("/\d/",$phone[0],$phonedata);
			$phone = implode($phonedata[0]);
			$house_info['owner_phone'] = $phone;
			// echo strrpos($source_url,'/');
		}
		//获取加密电话号码end

		//城区和商圈
		$html = file_get_contents($source_url);
		preg_match("/<i>(.*?) \- (.*?)<\/i>/",$html,$cityarea_id);
		$house_info['cityarea_id'] = $cityarea_id[1];
		$house_info['cityarea2_id'] = $cityarea_id[2];
		//城区和商圈end

		//房源描述
		preg_match("/desClose\">\s+([^<]+)/",$html,$house_des);
		$house_info['house_desc'] = trimall($house_des[1]);
		//房源描述end
