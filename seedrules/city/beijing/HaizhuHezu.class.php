...
/**
 * @description 北京嗨住 合租房抓取规则
 * @classname 北京嗨住
 */


Class HaizhuHezu  extends \city\PublicClass{
	...
	public function house_list($url){
	    $house_info = \QL\QueryList::run('Request', [
	        ...
	        'link' => ['li.cf > div:nth-child(2) > h2:nth-child(1) > a:nth-child(1)', 'href', '',],
	        'tag' => ['li.cf > div:nth-child(2) > p:nth-child(4) > span:nth-child(1)','text','',]
	        ])->getData(function($item){
	            if($item['tag'] == "合租"){
	...
	public function house_detail($url){
       $house_info = \QL\QueryList::run('Request',[
			...
			'house_room_totalarea' => ['.house-info > li:nth-child(2)', 'text', '', function($house_room_totalarea){
				return str_replace('m²', '', $house_room_totalarea);
			}],
			...
			'owner_phone' => ['.tel-text', 'text', '' ,function ($owner_phone){
			    ...
			    return str_replace('转',',',$owner_phone);
			}],
