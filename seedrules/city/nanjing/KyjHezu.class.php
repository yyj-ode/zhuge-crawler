...
/**
 * @description 南京快有家合租房源抓取规则
 * @classname 南京快有家合租(k-ok M-ok)
 */
...
Class KyjHezu extends \city\PublicClass{
    ...
    Public function house_page(){
         ...
         */
        //从首页获取最大列表页
        $url = "http://nj.kuaiyoujia.com/zufangs/hezuhouse/quyu";
        $maxPage = $this->getMaxPage($url);
        $urlarr = [];
        for($page = 1; $page <= $maxPage; $page ++) {
            $urlarr[] = "http://nj.kuaiyoujia.com/zufangs/hezuhouse/quyu-i".$page;
        }
        return $urlarr;
    }
    ...
    public function house_list($url){
        $house_info = \QL\QueryList::run('Request',[
            ...
            'link'=>['div.liebiao: > ul:nth-child(1) > a:nth-child(1)','href','',function($item){
                return "http://nj.kuaiyoujia.com".$item;
            }]
