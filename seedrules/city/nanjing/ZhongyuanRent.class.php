...
Class ZhongyuanRent extends \city\PublicClass
{
	...
	Public function house_page(){
		...
		for ($page=1; $page<$maxPage; $page++) {
			$urlarr[]= "http://mobileapi.centanet.com/025/api/Post?pagecount=10&startindex=".($page*10)."&posttype=r";
			}
		return $urlarr;
	}
	/*
	...
	public function house_list($url){
	    ...
	    foreach($list_decode['Result'] as $v){
	        $house_info[$k]= "http://nj.centanet.com/zufang/".$v['Id'].".html";
	   		$k++;
	    }
	    return $house_info;
	}
	...
	public function house_detail($url){
	    ...
	    $result_temp=json_decode(getXmlJsonSnoopy("http://mobileapi.centanet.com/025/api/Post?PostId=".$value),1);
	    //一个页面同步加载延迟0.02S
	    usleep(20000);
	    $result_image_array=json_decode(getXmlJsonSnoopy("http://mobileapi.centanet.com/025/api/PostImg?PostId=".$value),1);
	    
	    $result_image=$result_image_array['Result'];
	    $result=$result_temp['Result'];
	    $result_agen=json_decode(getXmlJsonSnoopy("http://mobileapi.centanet.com/025/api/Staff?staffNo=".$result['StaffNo']."&postId=".$value),1);
	    $house_info['app_url'] = "http://mobileapi.centanet.com/025/api/Staff?staffNo=".$result['StaffNo']."&postId=".$value;
	    //从web补充建筑年代数据、小区、城区
	    
	    //dump($result_html);die;
	    $result_html=file_get_contents($url);
	    ...
	    $house_info['house_room_totalarea'] = '';
