...
Class FangRent extends \city\PublicClass
{
    Public function house_page(){
        ...
        foreach($dis as $index){
            $URLPRE = "http://zu.gz.fang.com/house-a0".$index."/a21-n31/";
            //获取最大页
            $maxPage = $this->get_maxPage($URLPRE);
            echo $URLPRE."\r\n";
            $urlarr = [];
            for($page=1; $page<=$maxPage; $page++){
                $urlarr[] = ($page == 1) ? $URLPRE : "http://zu.gz.fang.com/house-a0".$index."/a21-i3".$page."-n31/";
            }
        }
        return $urlarr;
	...
	*/
	public function house_detail($source_url) {
        ...
        $source_url = $split[0];
	    //$source_url = "http://zu.gz.fang.com/chuzu/3_171295782_1.htm";
		//详情页拿信息
		...
		if(preg_match('/comName:\s*\'-1\'/u',$html)){
            ...
            if(empty($owner_name)){
                preg_match("/agentMobile: \'(\d+-\d+-\d+转\d+)\'/u",$info,$owner_phone);
                $owner_phone[1] = str_replace("转",",",$owner_phone[1]);
            }
