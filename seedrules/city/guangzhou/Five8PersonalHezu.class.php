...
Class Five8PersonalHezu extends \city\PublicClass  {
	...
	public function house_page(){
		...
		foreach($dis as $index){
            $URLPRE = "http://m.58.com/".$index."/hezu/0/?refrom=wap";
            //获取最大页
            $maxPage = $this->get_maxPage($URLPRE);

            for($page=1; $page<=$maxPage; $page++){
                $urlarr[] = 'http://m.58.com/'.$index.'/hezu/0/pn'.$page.'?refrom=wap';
            }
        }
	...
	*/
	public function house_list($url){
		...
		foreach ($ids as $k=>$v){
			$house_info[] = "http://m.58.com/gz/hezu/".$v."x.shtml";
		}
