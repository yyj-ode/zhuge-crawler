...
class Yitong58 extends \city\PublicClass
{
    ...
    public function house_page()
    {
        ...
        foreach ($this->dis as $k => $v) {
            ...
            foreach ($data as $key => $value) {
                ...
                for ($i=20; $i <= 70; $i++) {
                    $urlarr[] = $urlstr1 . $i . '/?key=%2525u58F9%2525u5F64%2525u5730%2525u4EA7&ClickID=1';
                }
            }
        }
        return $urlarr;
    ...
    public function house_detail($source_url)
    {
        ...
        preg_match("/壹彤|yitong/",$html,$match);
        if(empty($match)){
            return false;
        }
        $house_info = [];
        ...
        if(empty($house_info['owner_name'])){
            preg_match("/联系人([\x{0000}-\x{ffff}]+?)<\/a>/u",$html,$arr);
            $arr = trimall($arr[1]);
            $arr = strip_tags($arr);
            $house_info['owner_name']=$arr;
        }
    ...
    {
        for ($i = 1; $i <= 70; $i++) {
            $house_info[] = 'http://' . $this->city_name . '.58.com/ershoufang/1/pn' . $i . '/?key=%2525u58F9%2525u5F64%2525u5730%2525u4EA7&ClickID=1';
        }
        if (!$house_info)
            writeLog('Five8Personal' . __FUNCTION__, ['url' => $house_info], $this->log);
        return $house_info;
