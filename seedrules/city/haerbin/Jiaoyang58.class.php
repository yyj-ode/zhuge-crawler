...
class Jiaoyang58 extends \city\PublicClass
{
    ...
    public function house_page()
    {
        ...
        foreach ($this->dis as $k => $v) {
            ...
            foreach ($data as $key => $value) {
                ...
                for ($i=1; $i <= 70; $i++) { 
                    $urlarr[] = $urlstr1 . $i . '/?key=%2525u9A84%2525u9633%2525u5730%2525u4EA7%2525u4E8C%2525u624B%2525u623F&ClickID=1';
                }
            }
        }
        return $urlarr;
    ...
    Public function house_list($url = '')
    {
        ...
        preg_match("/没有找到您筛选条件下的信息/",$html,$notFound);
        if(empty($notFound)){
            return $lists;
        }
    }
    ...
    public function house_detail($source_url)
    {
        ...
        preg_match("/骄阳|jiaoyang/",$html,$match);
        if(empty($match)){
            return false;
        }
        $house_info = [];
        ...
        $house_info['source_name'] = '骄阳地产58';
        if(empty($house_info['cityarea2_id'])){
            preg_match("/位置：([\x{0000}-\x{ffff}]+?)（/u",$html,$arr);
            $arr = strip_tags($arr[1]);
            $arr = trimall($arr);
            $arr = explode("&nbsp;-",$arr);
            $house_info['cityarea_id'] = $arr[0];
            $house_info['cityarea2_id'] = $arr[1];
            $house_info['borough_name'] = $arr[2];
        }
        if(empty($house_info['owner_name'])){
            preg_match("/class=\"jjreninfo_des_jjr\"([\x{0000}-\x{ffff}]+?)<\/span>/u",$html,$arr);
            $arr = trimall($arr[1]);
            $arr = strip_tags($arr);
            $house_info['owner_name']=$arr;
        }
        if(empty($house_info['owner_phone'])){
        preg_match("/id=\"t_phone\">([\x{0000}-\x{ffff}]+?)</u",$html,$arr);
        $house_info['owner_phone']=trimarr($arr(1));
        }
        // $rules = array('text' => array('#broker_corp','text'));
        // $info = \QL\QueryList::Query($html,$rules)->data[0]['text'];
        // $house_info['etl'] = $info;
        return $house_info;
    }
    ...
    {
        for ($i = 1; $i <= 70; $i++) {
            $house_info[] = 'http://' . $this->city_name . '.58.com/ershoufang/1/pn' . $i . '/?key=%2525u9A84%2525u9633%2525u5730%2525u4EA7%2525u4E8C%2525u624B%2525u623F&ClickID=1';
        }
        if (!$house_info)
            writeLog('Five8Personal' . __FUNCTION__, ['url' => $house_info], $this->log);
        return $house_info;
