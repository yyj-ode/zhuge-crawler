...
/**
 * @description 哈尔滨赶集网 整个人房源抓取规则
 * @classname 哈尔滨赶集网
 */
class GjPersonal extends \city\PublicClass
{
    public $PRE_URL = 'http://hrb.ganji.com/fang5/';
    private $hhhhhh = '';
    private $_log = false;
    public $current_url = '';
    /**
    ...
    public function house_page()
    {
        ...
        $cityareas = ['nangang','daoli','daowai','xiangfang','songbei','kaifaqu','pingfang','acheng',
            'shuangcheng','shangzhi','wuchang','yilan','fangzheng','binxian','bayan',
            'mulan','tonghe','yanshou','hulan'];
        $urlarr = [];
        foreach($cityareas as $cityarea){
            ...
            $maxPage = ceil($maxPage[1]/42);
            if($maxPage > 63) {
                for ($n = 1; $n <= 2; $n++) {
                    if ($n == 1) {
                        $url = $this->PRE_URL.$cityarea.'/a1b0e80';
                    } else {
                        $url = $this->PRE_URL.$cityarea.'/a1b80e99999';
                    }
                    $html = getSnoopy($url);
                    if(empty($html)){
                        $html = getHtml($url);
    ...
    public function house_list($url)
    {
        ...
        //http://hrb.ganji.com/fang5/nangang/a1o1
        //http://hrb.ganji.com/fang5/nangang/a1o2
        //!!!!!!!!!!记得到时候试一下那个长的url
...
//            [11] => http://hrb.ganji.com/fang5/2359669945x.htm
//            [12] => http://hrb.ganji.com/fang5/2359684177x.htm
//            [13] => http://hrb.ganji.com/fang5/2376835562x.htm
//            [14] => http://hrb.ganji.com/fang5/2178553144x.htm
//            [15] => http://hrb.ganji.com/fang5/2359264441x.htm
//            [16] => http://hrb.ganji.com/fang5/2359255678x.htm
//            [17] => http://hrb.ganji.com/fang5/2359251490x.htm
//            [18] => http://hrb.ganji.com/fang5/2351105883x.htm
//            [19] => http://hrb.ganji.com/fang5/2348050871x.htm
//            [20] => http://hrb.ganji.com/fang5/2222164216x.htm
        sleep(mt_rand(2, 5));
...
//                $title = strip_tags($title);
        ...
        //加判断house_title为空
        if(empty($house_info[0]['house_title'])){
            preg_match("/<em\s*class=\"em-mqrz\">([\x{0000}-\x{ffff}]+?)<i/u", $html, $house_title);
            $house_info[0]['house_title'] = strip_tags($house_title[1]);
        }
        ...
        if($house_info[0]['house_room']>21){
            preg_match("/(\d+)室(\d+)厅/", $html, $house_room);
            $house_info[0]['house_room']=empty($house_room)?0:$house_room[1];
        }
        //几厅
        ...
        if($house_info[0]['house_toilet']>21){}
        //装修程度
