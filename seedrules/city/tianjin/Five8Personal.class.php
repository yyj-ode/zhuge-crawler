...
class Five8Personal extends \city\PublicClass
{
    ...
    public $city_name = 'tj';
    public $dis = array('nankai', 'hexi', 'hedong', 'heping', 'hebeiqu', 'hongqiaotj', 'tanggu', 'donglitj', 'xiqingtj', 'beichentj', 'jinnantj', 'tjkaifaqu', 'dagang', 'hangu', 'wuqing', 'baodi', 'jinghaiqu', 'ninghexian', 'jianxiantj', 'tianjin'
    );
    ...
    public function house_detail($source_url)
    {
        ...
        $html1 = file_get_contents($source_url);
        ...
        //楼层
        \QL\QueryList::Query($html, [
            'floor' => ['ul.infor-other > li:eq(2) ', 'text']
        ])->getData(function ($item) use (&$house_info) {
        	// var_dump($item['floor']);die;
            if ($item['floor']) {
                preg_match('/(.+)\(.*(\d*).*\)/', $item['floor'], $match);
                $house_info['house_floor'] = trim($match[1]);
                $house_info['house_topfloor'] = $match[2];
            }
        });
      
        preg_match('/<li\s*class=\"des_cols2\">(.*?)层\(共(.*?)层\)<\/li>/', $html1,$topfloor);
        $house_info['house_topfloor'] = $topfloor[2];
        // var_dump($topfloor[1]);die;
        // preg_match('/(.*?)层/', $topfloor[1],$house_floor);
        $house_info['house_floor'] = $topfloor[1];
        // var_dump(empty($topfloor));die;
        if(empty($topfloor)){
        	preg_match('/<li\s*class=\"des_cols2\">共(.*?)层<\/li>/', $html1,$topfloor);
        	$house_info['house_topfloor'] = $topfloor[1];
        	$house_info['house_floor'] = '';
        }
        // var_dump($topfloor);die;
        //城区，商圈
        preg_match("/位置：([\x{0000}-\x{ffff}]+?)<\/li>/u", $html1, $area);
        ...
        //图片
        preg_match_all('/<div\s*class=\"descriptionImg\">\s*<img\s*src=\"(.*?)\".*alt=\".*\"\/>\s*<\/div>/', $html1,$pics);
        $house_info['house_pic_unit'] = implode('|', $pics[1]);
