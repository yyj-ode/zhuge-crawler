...
/**
 * @description 上海我爱我家地产二手房抓取规则（说明：我爱我家源码中有乱码，不能直接用正则匹配，先使用explode函数拆分）
 * @classname 上海我爱我家
 */
...
//                    echo $surl."\r\n";
                    ...
                    \QL\QueryList::Query($html,[
                        "totalNum" => ['.font-houseNum','text'],
                        "list" => ['ul.list-body > li > a','href'],
                    ])->getData(function($data)use(&$resultNum){
                        if(isset($data['totalNum'])) $resultNum['totalNum'] = intval($data['totalNum']);
                        if($data['list']) $resultNum['list'][] = $data['list'];
                        return $data;
                    });
                    if($resultNum['totalNum'] === 0) continue;
                    $total += $resultNum['totalNum'];
                    //if( count($resultNum['list']) > 12){ }
    ...
    */
    public function house_list($url){
        ...
        foreach($hrefs[1] as $k=>$v){
            $house_info[$k] = 'http://sh.5i5j.com'.$v;
        }
    ...
    */
    ...
    public function house_count(){
        $PRE_URL = 'http://sh.5i5j.com/exchange/';
        $totalNum = $this->queryList($PRE_URL, [
            'total' => ['.font-houseNum','text'],
        ]);
        return $totalNum;
        // 	    return 0;
    }
    ...
    public function callNewData(){
        $resultData = [];
        for($i = 1; $i <= 100; $i++){
            $resultData[] = "http://sh.5i5j.com/exchange/o6n{$i}";
        }
