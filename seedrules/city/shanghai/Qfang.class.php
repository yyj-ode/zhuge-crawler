...
Class Qfang extends \city\PublicClass
{
    public function house_page(){
        $gzurl = "http://shanghai.qfang.com/appapi/v3/room/list?bizType=SALE&currentPage=1&dataSource=shanghai&pageSize=20";
        $api = getHtml($gzurl);
        $apiarr = json_decode($api,true);
        $maxPage=$apiarr['result']['pageCount'];
        $urlarr = [];
        for($page=1; $page<=$maxPage; $page++){
            $urlarr[] = "http://shanghai.qfang.com/appapi/v3/room/list?bizType=SALE&currentPage=".$page."&dataSource=shanghai&pageSize=20";
        }
        return $urlarr;
    }
	/*
	...
	public function house_list($url){
        ...
        if(preg_match('/^f\d+$/', $baseName)){
            ...
            \QL\QueryList::Query($html,[
                ...
                $item['list'] && $lists[] = 'http://shanghai.qfang.com' . $item['list'];
            });
        }else{
            ...
            foreach($list as $element){
                $id=$element['id'];
                $lists[]="http://shanghai.qfang.com/sale/".$id;
            }
        }
	...
	*/
	public function house_detail($source_url){
        ...
        $apiUrl = "http://shanghai.qfang.com/appapi/v3/room/detail?bizType=SALE&dataSource=shanghai&id=".$id."&pageSize=20&qchatPersonId=60057&which=5";

        // api接口
        $requestApi = json_decode(getHtml($apiUrl), true);
		$result_2=$requestApi['result'];
        if( !preg_match('/Q房网·上海/u', $result_2['broker']['company']) ){
...
//        $house_info['off_type'] = $this->is_off($house_info['source_url'],$pc_html);
    ...
    public function callNewData(){
        $resultData = [];
        for($i = 1; $i <= 100; $i++){
            $resultData[] = "http://shanghai.qfang.com/sale/f{$i}";
        }
