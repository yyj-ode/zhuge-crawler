...
Class FangHezu extends \city\PublicClass{
    ...
    Public function house_page(){
        ...
        //每个城区列表页的最大页码
        $pagelist = array(
            100,46,25,5,4,20,10,100,23,26,48,64,63,10,10,2,70,1,2);
        //0-19
        $index = empty($_GET['dis'])?0:$_GET['dis'];
        $flag_page = 0;
        $URLPRE = "http://zu.sh.fang.com/hezu-a0".$dis[$index]."/h31";
	...
	*/
	public function house_list($url){
		...
		foreach ($hl[1] as $k=>$v){
            $house_info[$k] = "http://zu.sh.fang.com".$v.'|'.$house_floor[$k].'|'.$house_topfloor[$k];
		}
        return $house_info;
	}
	...
	*/
	public function house_detail($source_url) {
        //$source_url = 'http://zu.sh.fang.com/chuzu/15_911322_-1.htm';
        $split = explode('|',$source_url);
        $source_url = $split[0];
		preg_match("/\_(\d+)\_/u", $source_url, $id);
		$web_url = "http://m.fang.com/zf/sh/AGT_".$id[1].".html";
		$html = gb2312_to_utf8(getSnoopy($source_url));
        $add = gb2312_to_utf8(getSnoopy($web_url));
        //下架检测
//        $house_info['off_type'] = $this->is_off($source_url,$html);
        ...
        if ($comName[1] == "-1"){
            //详细信息
            //标题
            preg_match("/<span\s*class=\"bold\">[\x{0000}-\x{ffff}]+?<\/span>/u", $html, $title);
            if(!isset($title[0])){
                preg_match("/<dl\s*class=\"title\">[\x{0000}-\x{ffff}]+?<\/p>/u", $html, $title);
            }
            if(!isset($title[0])){
            ...
            preg_match("/<div\s*class=\"info\s*floatr\">[\x{0000}-\x{ffff}]+?<\/div>/u", $html, $info);
            if(!isset($info[0])){
...
// 		    var_dump($info);
            ...
            preg_match("/房屋[\x{0000}-\x{ffff}]*?合租/u", $info, $a);
            preg_match("/:(\d+)户合租/u", $info, $homes);
            preg_match("/家具家电:([\x{0000}-\x{ffff}]+?)评分/u", $info, $config);
            ...
            preg_match("/家具家电:([\x{0000}-\x{ffff}]+?)配套设施:([\x{0000}-\x{ffff}]+?)评分/u", $info, $config);

            $house_configroom = (preg_match("/暂无信息/", $config[1]) ? '':$config[1].',') . (preg_match("/暂无信息/", $config[2]) ? '':$config[2]);

            $house_configroom = preg_replace("/\/[\x{0000}-\x{ffff}]*?,/u", ",", $house_configroom);
            $house_configroom = preg_replace("/\/[\x{0000}-\x{ffff}]+/u", "", $house_configroom);

            if(empty($house_configroom)){
                $house_info['house_configroom'] = '';
            }else{
                $house_info['house_configroom'] = str_replace(',', '#', $house_configroom);
            }
