<?php namespace tianjin;
     ...
     * @classname 天津赶集网
     */
class GjPersonal_2 extends \city\PublicClass
{
    public $PRE_URL = 'http://su.ganji.com/fang5/a1/';
    private $hhhhhh = '';
    private $_log = false;
    public $current_url = '';
    /**
    ...
    public function house_page()
    {
        ...
        $cityareas = [
            'heping','hedong','hexi','nankai','hebei','hongqiao', 'tanggu',
            'hangu', 'dagang','kaifaqu','dongli','xiqing','beichen','jinnan',
            'wuqing', 'baodi', 'jixian','jinghai','ninghe','binhaixinqu'
        ];
        $urlarr = [];
...
//            sleep(mt_rand(2, 5));
            ...
            $maxPage = ceil($maxPage[1]/31);
            $urlarr []= $this->getExtensivePage(
                $minPrice=0, $maxPrice=128, $maxPage, $circleTime=0, $cityarea
            );
        }
        $urlarrs = [];
        foreach($urlarr as $v1 ){
            foreach($v1 as $v2){
                if(is_string($v2)){
                    $urlarrs[] = $v2;
                }
...
//        dumpp($resultData);die;
    ...
    public function getExtensivePage($minPrice,$maxPrice,$maxPage,$circleTime,$cityarea){
        $gt63 = $maxPage;
        $price = [];
        $urlarr = [];
        if($maxPage >= 64) {
            $price[0] = $minPrice;
            $price[1] = ($maxPrice-$minPrice)/2;
            $price[2] = $maxPrice;
            for ($n = 0; $n <= 1; $n++) {
                $url = $this->PRE_URL.$cityarea.'/a1b'.$price[$n].'e'.$price[$n+1];
                $html = getSnoopy($url);
                if(empty($html)){
                    $html = getHtml($url);
                }
                ...
                $maxPage = ceil($maxPage[1] / 31);
                $urlarr[] = $this->getExtensivePage(
                    $price[$n],$price[$n+1],$maxPage,++$circleTime,$cityarea
                );
            }
        } else {
            if($circleTime==0){
                for($i = 1;$i<=$maxPage;$i++){
                    $urlarr[][] = $this->PRE_URL.$cityarea.'/a1o'.$i;
                }
            } else {
                for($i = 1; $i <= $maxPage; $i++){
                    $urlarr[] = $this->PRE_URL.$cityarea.'/a1b'.$minPrice.'e'.$maxPrice.'o'.$i;
                }
            }
        }
        if($gt63 > 63)
        {
            $url = $this->PRE_URL.$cityarea.'/a1b128e99999';
            $html = getSnoopy($url);
            if(empty($html)){
                $html = getHtml($url);
            }
