...
class FangPersonal  extends \city\PublicClass
{
    ...
    public  $kunshan_host = 'http://esf.ks.fang.com';
    private $tag = [
        '满二',
        '满五唯一',
        '学区房',
    ...
    public function house_page()
    {
        $urls=[];
        //除昆山之外的部分
        $url = $this ->host.'/house/a211';
        $queryData = $this -> getHtmlQueryData($url);
        foreach($queryData['dis'] as $dis){
            $url = $this ->host.'/'.$dis.'/a211/';
            $queryData = $this -> getHtmlQueryData($url);
            for($page=1;$page<=$queryData['maxPage'];$page++){
                $urls[] = $this ->host.'/'.$dis.'/'.'a211-i3'.$page;
            }
        }
        //昆山部分
        $url = $this->kunshan_host.'/house/a211';
        $queryData = $this -> getHtmlQueryData($url);
        foreach($queryData['dis'] as $dis){
            $url = $this ->kunshan_host.'/'.$dis.'/a211/';
            $queryData = $this -> getHtmlQueryData($url);
            if($queryData['maxPage'] == 100){
                foreach($queryData['plate'] as $plate){
                    $url = $this ->kunshan_host.'/'.$dis.'-'.$plate.'/a211/';
                    $queryData = $this -> getHtmlQueryData($url);
                    for($page=1;$page<=$queryData['maxPage'];$page++){
                        $urls[] = $this ->kunshan_host.'/'.$dis.'-'.$plate.'/'.'a211-i3'.$page;
                    }
                }
            } else {
                for($page=1;$page<=$queryData['maxPage'];$page++){
                $urls[] = $this ->kunshan_host.'/'.$dis.'/'.'a211-i3'.$page;
                }
            }
        }
        return $urls;
...
//        var_dump($info);die;
        ...
        if(empty($house_info['owner_name'])){
            preg_match("/\d业主(.*)评估/u", $info, $owner_name);
            if($owner_name){
    ...
    protected function getHtmlQueryData($url){
        ...
        //最大页数
        preg_match("/class=\"fy_text\">1\/(\d+)</",$html,$match);
        if($match[1]){
            $queryData['maxPage'] = $match[1];
        }
        //  城区
