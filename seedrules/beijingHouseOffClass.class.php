...
//        var_dump($this->content);die;
        preg_match('/您浏览的网页暂时不可用/', $this->content, $ID);
        $value = $ID[0];
//        usleep(20000);
        if ($value) {
            $this->settingHouseOff();
    ...
    private function QfangRent()
    {
    	...
    	if(strstr($url,"qchatPersonId")){
    		...
    		$content = getHtml($url);
    		$tag = \QL\QueryList::Query($content, [
    				"isOff" => ['.tips-hs-crtstatus', 'class', ''],
    				])->getData(function ($item) {
    					return $item;
    				});
    				if ($tag) {
...
//             'house-shelves' => ['.removeIcon', 'class', ''],
        	...
        	'list'=>['li.on > a:nth-child(1) > span:nth-child(1)','text','',function($item1){
        		return preg_match("/全部租房/", $item1);
        	}],
        ])->getData();
        if ($Tag) {
            $this->settingHouseOff();
            if ($Tag[0]['disparue'] ||!empty($Tag[0]['unshelf'])|| !empty($Tag[0]['list'])) {
            	
//             if (!empty($Tag[0]['house-shelves']) || $Tag[0]['disparue'] ||!empty($Tag[0]['unshelf'])|| !empty($Tag[0]['list'])) {            	
    ...
    {
        $Tag = \QL\QueryList::Query($this->content, [
            "ischengjiao" => ['.big_qvkuai_top', 'text', '', function ($item) {
                return preg_match("/您访问的房源已售出/", $item);
            }],
            'isOff'=>['.contenttop_err','text','',function($item){
            	return preg_match("/您访问的页面不存在/", $item);
            }],
        ])->getData();
        if ($Tag) {
            $this->settingHouseOff();
            if (!empty($Tag[0]['isOff'])) {
                $this->off_reason = 1;  //下架
            } else {
                $this->off_reason = 2;  //成交
            }
