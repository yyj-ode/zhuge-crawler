...
include_once '../common/common.php';
    ...
    $d[] = '-help 帮助';
    $d[] = '-n 并发数 默认为50';
    $d[] = '-diff <状态> 是否比较房源变化(yes 是 no 否 默认为 no)';
 ...
 */
$serv->on('receive', function ($serv, $fd, $from_id, $data) use($n, $diff, $port){
    $data = json_decode($data, true);
    if($data['instructions'] == 'stop'){ return false;}
    getRedisInit();
    $serv->tick(2000, function() use ($serv, $fd, $from_id, $data, $n, $diff, $port) {
        $typenum = $data['type'].$port.'as';
        $mantypenum = $typenum.'num';
        if($concurrentnum = handleCheckThreadNum($typenum, $n)){
            for($i = 0; $i <= $concurrentnum; $i++){
                usleep(200);
                $task_id = $serv->task(json_encode(['fd' => $fd, 'type' => $typenum, 'diff' => $diff]));
            }   
        }else{
            $redis = getRedisInit();
